...
import random
import skills
from skills import Skill  # 导入技能基类
import effects  # 导入效果模块（包含具体效果类）
from effects import Effect  # 导入效果基类
...
class Pokemon:
    ...
    def heal_self(self, amount):
        ...
        if self.hp > self.max_hp:
            self.hp = self.max_hp  # 确保生命值不超过最大值
    ...
    def on_evade(self, attacker: Pokemon):
        ...
        :param attacker: 发起攻击的宝可梦对象（可能为 None）
        """
        return
    ...
    def add_status_effect(self, effect: Effect):
        ...
        由于传入的是 Effect 的实例，无法区分调用者是否显式传入 level=1 还是使用默认值 1
    ...
    def apply_status_effect(self):
        ...
        使用切片创建列表副本进行遍历，避免在遍历过程中修改列表导致的问题
        """
