...
class Effect:
    ...
    def __init__(self, duration: int, level: int | None = None) -> None:
        ...
        # 区分调用者是否显式传入了 level 参数。
        # 当 level 为 None 时视为未显式传入（仅刷新持续时间的场景）；
        # 当 level 非 None 时视为显式传入，即使传入的是 1 也应被视为显式。
...
class PoisonEffect(Effect):
    ...
    def __init__(self, damage: int = 10, duration: int = 3) -> None:
        # 初始化中毒效果
        # damage: 每回合造成的伤害百分比（整数），例如 10 表示按当前生命的 10% 造成伤害
    ...
    def apply(self, pokemon: "Pokemon") -> None:
        ...
        # 传入的 self.damage 被视为百分比整数（例如 10 表示 10%）
        pre_hp = pokemon.hp
...
# 百分比减伤效果类，继承自Effect基类
class DamageReductionEffect(Effect):
    name = "百分比减伤提升"  # 效果名称为"百分比减伤"
...
# 攻击力增强效果类，继承自Effect基类
class AttackBoostEffect(Effect):
    name = "攻击力增强"  # 效果名称为"攻击力增强"

    def __init__(self, duration: int, level: int | None = None) -> None:
        # duration: 持续回合数；
        # level: 若显式传入（即不为 None），表示希望叠加该层数；若不传入（None），视为仅添加/刷新持续时间，不表示要叠加层数。
